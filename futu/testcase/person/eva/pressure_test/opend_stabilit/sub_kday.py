...
class SubKDay(object):
    '''
    订阅实时K线：日k，股票个数：用满500个订阅额度为止
    '''
    def sub(self):
        #日志
        ...
        #行情上下文实例
        # quote_ctx = futuquant.OpenQuoteContext(host= host, port= port_kday )
        quote_ctx = futuquant.OpenQuoteContext(host='127.0.0.1', port=11111)
        quote_ctx.start()
        # 设置监听
        handler = CurKlineTest()  #日K
