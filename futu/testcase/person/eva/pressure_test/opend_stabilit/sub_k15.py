...
class SubK15M(object):
    '''
    订阅实时K线：15分K，股票个数：用满500个订阅额度为止
    '''
    def sub(self):
        #日志
        ...
        #行情上下文实例
        quote_ctx = futuquant.OpenQuoteContext(host= host, port= port_k15 )
        quote_ctx.start()
        # 设置监听
        handler = CurKlineTest()  #15分K
