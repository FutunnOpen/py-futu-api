...
class GetCurKline(unittest.TestCase):
    ...
    def step_check_base2(self, casename, code, num, ktype, autype):
        ...
        :return:
        '''
        ret_code_sub, ret_data_sub = quote_ctx.subscribe(code, ktype)
        ret_code, ret_data = quote_ctx.get_cur_kline(code, num, ktype, autype)
        print(casename, file=f, flush=True)
        print(ret_data, file=f, flush=True)
        # 校验
    ...
    def test_get_cur_kline_bk_stock_unsubscribe_bk(self):
        ...
        quote_ctx.subscribe(code_list,SubType.K_1M)
        ret_code, ret_data = quote_ctx.get_cur_kline(code_list[0],10,SubType.K_1M)
        print(casename, file=f, flush=True)
        print(ret_data, file=f, flush=True)
        ret_code, ret_data = quote_ctx.get_cur_kline(code_list[1],10,SubType.K_1M)
        print(ret_data, file=f, flush=True)
        time.sleep(60)
        quote_ctx.unsubscribe('SH.BK0001',SubType.K_1M)
        ret_code, ret_data = quote_ctx.get_cur_kline(code_list[0],10,SubType.K_1M)
        print(ret_data, file=f, flush=True)
    ...
    def test_get_cur_kline_bk_stock_unsubscribe_stock(self):
        casename = sys._getframe().f_code.co_name
        code_list = ['HK.00700','SH.BK0001']
        quote_ctx.subscribe(code_list,SubType.K_5M)
        ret_code, ret_data = quote_ctx.get_cur_kline(code_list[0],10,SubType.K_5M)
        print(casename, file=f, flush=True)
        print(ret_data, file=f, flush=True)
        ret_code, ret_data = quote_ctx.get_cur_kline(code_list[1],10,SubType.K_5M)
        print(ret_data, file=f, flush=True)
        time.sleep(60)
        quote_ctx.unsubscribe('HK.00700',SubType.K_5M)
        ret_code, ret_data = quote_ctx.get_cur_kline(code_list[1],10,SubType.K_5M)
        print(ret_data, file=f, flush=True)
...
class CureKlineTest(CurKlineHandlerBase):
    # 监听实时K线推送
    def on_recv_rsp(self, rsp_str):
        ...
        :return:
        '''
        ret_code, ret_data = super(CureKlineTest, self).on_recv_rsp(rsp_str) # 基类的on_recv_rsp方法解包返回了K线信息
        #打印,记录日志
        print(ret_data, file=f, flush=True)
