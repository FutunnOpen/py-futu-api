...
class GetStockQuote(unittest.TestCase):
    ...
    def step_check_base1(self, casename, code_list):
        ...
        print(casename)                                                                                # 原因是反订阅前得等待60s，如果每个用例都等待60s自动化时间将大大加长，效率低。
        ret_code, ret_data1 = quote_ctx1.get_stock_quote(code_list)
        ...
        ret_code, ret_data2 = quote_ctx2.get_stock_quote(code_list)
    ...
    def test_get_stock_quote_bk_stock_unsubscribe_bk(self):
        ...
        ret_code, ret_data1 = quote_ctx1.get_stock_quote('HK.00700')
        print(ret_data1, file=f1,flush=True)
        quote_ctx2.unsubscribe('SH.BK0001', SubType.QUOTE)
        ret_code, ret_data2 = quote_ctx2.get_stock_quote('HK.00700')
        print(ret_data2, file=f2, flush=True)
        print(CompareData().compare(ret_data1, ret_data2))
    ...
    def test_get_stock_quote_bk_stock_unsubscribe_stock(self):
        ...
        ret_code, ret_data1 = quote_ctx1.get_stock_quote('SH.BK0001')
        print(ret_data1, file=f1,flush=True)
        quote_ctx2.unsubscribe('HK.00700', SubType.QUOTE)
        ret_code, ret_data2 = quote_ctx2.get_stock_quote('SH.BK0001')
        print(ret_data2, file=f2, flush=True)
        print(CompareData().compare(ret_data1, ret_data2))
...
#         # print(casename, file=f2, flush=True)
#         #设置监听
#         handler1 = StockQuoteTest1()
#         handler2 = StockQuoteTest2()
...
#
#
# class StockQuoteTest1(StockQuoteHandlerBase):
...
#         :return:
#         '''
#         ret_code, data = super(StockQuoteTest1,self).on_recv_rsp(rsp_str) # 基类的on_recv_rsp方法解包返回了报价信息
#
#         print(data, file=f1,flush=True)
#         return RET_OK, data

# class StockQuoteTest2(StockQuoteHandlerBase):
#     def on_recv_rsp(self, rsp_str):
#         '''
#         回调
#         :param rsp_str:
#         :return:
#         '''
#         ret_code, data = super(StockQuoteTest2,self).on_recv_rsp(rsp_str) # 基类的on_recv_rsp方法解包返回了报价信息
#
...
if __name__ == '__main__':
    ...
    quote_ctx2 = OpenQuoteContext('127.0.0.1',11112)
    pandas.set_option('max_columns', 100)
    pandas.set_option('display.width', 1000)
    pandas.set_option('display.max_rows', 1000)
    code = 'US.VIPS'
    subtype=SubType.QUOTE
    quote_ctx1.subscribe(code, subtype)
    quote_ctx2.subscribe(code, subtype)
    print(quote_ctx1.get_stock_quote(code))
    print(quote_ctx2.get_stock_quote(code))
    # tmp = GetStockQuote()
    # tmp.test_get_stock_quote_bk_stock_unsubscribe_bk()

    # # df_tmp = pandas.DataFrame(ret_data)
    # col = list(ret_data.columns)
    # col.append('push_data_type')
    # df_datas1 = pandas.DataFrame(columns=col)
    # df_datas2 = pandas.DataFrame(columns=col)
