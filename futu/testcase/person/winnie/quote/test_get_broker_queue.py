...
f1 = open('test_broker_queuel_low.txt','a')
f2 = open('test_broker_queue2_low.txt','a')
...
class GetBrokerQueue(unittest.TestCase):
    ...
    def common_get_broker_queue(self, code, case_name):
        ...
        ret_code, ret_data_big, ret_data_ask = quote_ctx1.get_broker_queue(code)
        print(ret_data_big, ret_data_ask, file=f1,flush=True)
        ret_code, ret_data_big, ret_data_ask = quote_ctx2.get_broker_queue(code)
        print(ret_data_big, ret_data_ask, file=f2, flush=True)
        if code is 'HK.00700':
            print(ret_data_big, ret_data_ask)
        return ret_code, ret_data_big, ret_data_ask
    ...
    def test_get_broker_queue_hk_stopped_stock(self):
        code = 'HK.01280'
        case_name = sys._getframe().f_code.co_name
        ret_code, ret_data_big, ret_data_ask = self.common_get_broker_queue(code, case_name)
        # 无法自动校验，仅能保证返回的数据是空
        # print(len(ret_data_big), len(ret_data_ask))  # 0 0
